...
$$\hat{\boldsymbol{\beta}}_{MAP} = ((\boldsymbol{X}'\boldsymbol{X} + A^{-1})^{-1} (\boldsymbol{X}'\boldsymbol{y} + A^{-1}\boldsymbol{m})$$- This MAP can be defined as a precision-weighted average of the estimate of $\boldsymbol{\beta}$ from the data and the estimate from the prior. Precision matrix is the inverse of covariance matrix.

###Example

Find the MAP of the insurance charges regression parameters assuming a normal prior with $\boldsymbol{m} = \boldsymbol{0}$ and $A = I$. A fairy weakly informative prior
...
### What to know from this module for THE FINAL 

1.  Fundamental difference between frequentist and Bayesian statistics
2.  Interpret a credible interval vs a confidence interval
3.  what happens when you change your prior... from the graph (weakly informative vs strong prior informative distributions) (play around with that shiny app)
