...
```{r}
# mod int defined previously is the full model 
mod_red <- lm(charges ~ bmi + smoker, health)
anova(mod_red, mod_int)
```

$F = 433.06, p < 2.2 \times 10^{-16}$ , we reject $H_o$ and conclude that there is a statistically significant difference in the linear relationship between bmi and health expenditures not covered by insurance when accounting for smoker status.
...
bptest(mod_int)
ks.test(rstandard(mod_int), "pnorm")

```

Clearly the assumption of normality is still violated, adding interaction terms only helps with effects of confounding, they are not designed to fix violations of model assumptions.
...
-   If you want to include interaction terms in the model, you should also include their associated first order terms. So the highest order must include the orders previous.

```{r}
# create a model with all first and second order terms for health expenditures 
# all variables confounding with all variables 

mod_all <- lm(charges ~ .*., health)
summary(mod_all)
```
