...
a.  State the regression line for the distribution's natural parameter in the context of the problem.
    ...
    $log(\hat{\lambda}) = -4.1632 - 1.08386 prog_{general} + -0.71405 prog_{vocational} + 0.07015 math$
...
d.  Perform a formal hypothesis test to determine if your model in (a) is statistically significant. Assume $\alpha = 0.05$.
    ...
    $X^2 = 98.223, p = 3.747 \times 10^{-21} < \alpha = 0.05$ , we reject $H_0$ and conclude that a poisson regression model predicting the expected number of awards a student has by the type of program a student is in and their math proficiency score is statistically significant.
...
e.  Perform a formal hypothesis test to determine if adding interaction terms between program and math score is statistically significant. Assume $\alpha = 0.05$.
    ...
    $X^2 = 0.348, p = 0.8403 > \alpha = 0.05$ , we fail to reject the null hypothesis and conclude adding interaction terms between program and math score is not statistically significant in our model predicting expected number of awards by program and math proficiency.
...
f.  Determine if the model you chose in (e) is a good fit for the data.
    ...
    I chose the original model I made due to interaction terms not being statistically significant. For the original model, it is clear that this is not a good fit for the data with all the points being concentrated to the left side of the graph. To say the model is a good fit for the data we would like to see the points randomly scattered across the plot, that is not happening, therefore this model is not a good fit for the data.
